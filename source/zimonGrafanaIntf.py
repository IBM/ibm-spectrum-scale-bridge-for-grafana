...
def bind_opentsdb_server(args):
    ...
    opentsdb_server._socket_host =  args.get('bindip', '0.0.0.0')
...
def bind_prometheus_server(args):
    ...
    prometheus_server._socket_host = args.get('bindip', '0.0.0.0')
    if args.get('protocol') == "https":
        certPath = os.path.join(args.get('tlsKeyPath'), args.get('tlsCertFile'))
        keyPath = os.path.join(args.get('tlsKeyPath'), args.get('tlsKeyFile'))
...
def main(argv):
    ...
    if (sys.version_info < (3, 11)):
        print(f'\nYor system running {sys.version} \n\nThe IBM Storage Scale bridge for Grafana requires Python3.11 or above. \
        \nRead the following instructions for possible solution: \
        \nhttps://github.com/IBM/ibm-spectrum-scale-bridge-for-grafana/wiki/What-to-do-if-your-system-is-on-a-Python-version-lower-than-3.11')
        return
